...
def _use_case(hw, quota_repo):
    ...
    return CreateBookingUseCase(booking_repo, image_repo, hw_repo, quota_repo, dispatcher=MagicMock())
...
async def test_ssd_config_counts_toward_ssd_quota_and_is_rejected():
    ...
    with patch("app.tasks.provision.provision_vm_task"):
        with pytest.raises(QuotaExceededError, match="SSD disk"):
            await uc.execute(AsyncMock(), ttl_minutes=60, image_id=uuid4(), hw_config_id=hw.id, user_id="test-user")
...
async def test_hdd_config_unaffected_by_ssd_usage():
    ...
    with patch("app.tasks.provision.provision_vm_task"):
        booking = await uc.execute(AsyncMock(), ttl_minutes=60, image_id=uuid4(), hw_config_id=hw.id, user_id="test-user")
