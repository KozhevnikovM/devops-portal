...
`hdd_mb`. A `{"hdd_mb": ...}` body was dropped as an unknown field (never reached the repo),
and a `{"disk_mb": ...}` body set a stray non-column attribute that never persisted. Either
way disk edits silently no-opped. After the fix the schema field is `hdd_mb`.
"""
...
def test_patch_hardware_persists_hdd_mb():
    app = _client()
    hw_id = uuid4()
    updated = _hw(51200)
    try:
        with patch("app.presentation.routes.api._hw_config_repo") as mock_repo:
            ...
            resp = TestClient(app).patch(f"/api/hardware/{hw_id}", json={"hdd_mb": 51200})

        assert resp.status_code == 200
        # The disk value reached the persistence layer under the column name `hdd_mb`.
        mock_repo.update.assert_awaited_once()
        _, _, fields = mock_repo.update.await_args.args
        assert fields == {"hdd_mb": 51200}
        assert resp.json()["hdd_mb"] == 51200
    finally:
