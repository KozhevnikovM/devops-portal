...
- The lease never starts while an order is still creating an environment's children. A persisted construction-complete marker is set once the last child exists, and every lease trigger, including reconciliation and queue promotion, requires it.
...
- One additive DB migration (`0032`): `environments.construction_complete`, with existing rows backfilled to `true`, so the lease never starts on a half-built environment. Deploying repairs existing orphaned environments, as described above, and the release notes must call this out.
