...
- The booking row no longer offers a **Release** action for an environment child. It points the user to the parent environment instead.
...
- The lease-start rule changes. The whole-stack lease starts once **every child has settled**: no child is QUEUED, PENDING, PROVISIONING, CONFIGURING or RETRY, and at least one child is READY. Before, the rule was "every child is READY". A child that ends in FAILED can therefore no longer keep the far-future placeholder forever, and TTL enforcement eventually tears down the live siblings. A lease that has already started is never pushed back by a later trigger. When a QUEUED pooled child is promoted, the environment's lease is now checked as well, which was a second way to leave the placeholder expiry in place.
